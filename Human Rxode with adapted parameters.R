...
BW      <- 81.6    #Body weight in Kg

#-Tissues volumes in % body weight-#

V_F      <- 21.04  #Fat
V_L      <- 1.773   #Liver
V_SI     <- 1.24   #Small intestine
V_A      <- 1.92   #Arterial Blood
V_V      <- 3.84   #Venous Blood
V_RP     <- 4.76   #Richly perfused 
V_SP     <- 47  #Slowly perfused 

#-Cardiac parameters-#

Q_C      <- 338.7    #Cardiac output in L/h

#-Blood flow to tissues in % cardiac output-#

Q_F      <- 5.16    #Fat
Q_L      <- 7.1   #Liver
Q_SI     <- 16.38    #Small intestine
Q_RP     <- 44.9  #Richly perfused (RP)
Q_SP     <- 35.38   #Slowly perfused (SP)
...
DOSE         <-(Dose_in_mg * BW)/ MW  * 1e+6     #The administered dose in umol 
