...
PBK_Cinnamaldehyde <- RxODE({
    ...
    RM_SI_CA       <- k_SI_CA * C_V_SI;                              #Rate of Cinnamaldehyde enzymatically oxidized cabroxylic acid in the small intestine in μmol/h
    ...
    RM_SI_AG_GST   <- Vsmax_SI_GST * C_V_SI * C_SIc_GSH / (Km_SI_GST_G * C_V_SI + Km_SI_GST * C_SIc_GSH + C_SIc_GSH * C_V_SI);  #-amount of cinnamaldehyde metabolized in the small intestine to GSH conjugate by GST in μmol
    RM_SI_AG_CHEM  <- k_GSH * C_V_SI * C_SIc_GSH * V_SI;                                       #Rate of Cinnamaldehyde binding in the small intestine to GSH in μmol/h
    RM_SIc_GSH     <- G_SYN_SI * V_SI * 0.9 - (RM_SI_AG_GST + RM_SI_AG_CHEM + k_SI_GLOS * AM_SIc_GSH);         #Rate of  GSH concentration in the Smal intesinte cytosol μmol/h
    ...
    RM_SI_AO       <- Vsmax_SI_AO * C_V_SI / (Km_SI_AO + C_V_SI);             #Ammount of Cinnamaldehyde reduced to cinnamyl alcOHol in the small intestine in μmol
    ...
    RM_L_AP        <- k_GSH * C_V_L * C_PRO_L * V_L;                                           #Rate of Cinnamaldehyde proteins adducts formation in the liver in μmol/h
    ...
    R_OH_L         <- Q_L * C_OH_A + Q_SI * C_OH_V_SI + RM_L_AO - (Q_L + Q_SI) * C_OH_V_L - R_OH_M_L_C_A;#Rate of in  Cinnamyl alcOHol concentration in the liver in μmol
    ...
    R_A            <- Q_C * C_A_Pu - (Q_F * C_A + Q_L * C_A + Q_SI * C_A + Q_RP * C_A + Q_SP * C_A);     #Rate of change in Cinnamaldehyde concentratin in the arterial blood in μmol 
    #Cinnamyl alcohol
    ...
    #-amount of Cinnamaldehyde in GI cavity in μmol
    ...
    d/dt(AM_L_AP)   <- RM_L_AP;       #Amount of Cinnamaldehyde proteins adducts in the liver in μmol
    ...
    d/dt(AM_SI_CA)      <- RM_SI_CA;      #Amount of Cinnamaldehyde enzymatically oxidized cabroxylic acid in the small intestine in μmol
