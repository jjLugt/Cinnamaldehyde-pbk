...
parameters=c("P_F", "P_L" ,"P_SI", "P_RP", "P_SP", "P_OH_F", "P_OH_L", "P_OH_SI", "P_OH_RP", "P_OH_SP",  "BW",  "V_F ",  "V_L ",  "V_SI ",  "V_A ",  "V_V ",  "V_SI ",  "V_RP ",  "V_SP ",  "Q_C ",  "Q_F ",  "Q_L ",  "Q_SI ",  "Q_RP ",  "Q_SP ",  "G_SYN_L ",  "G_SYN_SI ", 
             "k_L_GLOS ",  "k_SI_GLOS ",  "init_GSH_L ",  "init_GSH_SI ",  "k_GSH ",  "k_DNA ",  "C_PRO_L ",  "C_PRO_SI ",  "C_L_dG ",  "T_0.5 ",  "DOSE ",  "Ka ",  "Km_L_CA ", " Km_L_GST ",  "Km_L_GST_G ",  "Vsmax_L_CA ",  "Vsmax_L_AO ",
...
q <- c("qunif")
q.arg <- list(list(min = P_F / 2, max = P_F * 2),
              list(min = P_L / 2, max = P_L * 2),
              list(min = P_SI / 2, max = P_SI * 2),
              list(min = P_RP / 2, max = P_RP * 2),
              list(min = P_SP / 2, max = P_SP * 2),
              list(min = P_OH_F / 2, max = P_OH_F * 2),
              list(min = P_OH_L / 2, max = P_OH_L * 2),
              list(min = P_OH_SI / 2, max = P_OH_SI * 2),
              list(min = P_OH_RP / 2, max = P_OH_RP * 2),
              list(min = P_OH_SP / 2, max = P_OH_SP * 2),
              list(min = BW *0.9, max = BW *1.1),
              list(min = V_F / 2, max = V_F * 2),
              list(min = V_L / 2, max = V_L * 2),
              list(min = V_SI / 2, max = V_SI * 2),
              list(min = V_A / 2, max = V_A * 2),
              list(min = V_V / 2, max = V_V * 2),
              list(min = V_SI/ 2, max = V_SI * 2),
              list(min = V_RP / 2, max = V_RP * 2),
              list(min = V_SP / 2, max = V_SP * 2),
              list(min = Q_C / 2, max = Q_C * 2),
              list(min = Q_F / 2, max = Q_F * 2),
              list(min = Q_L / 2, max = Q_L * 2),
              list(min = Q_SI / 2, max = Q_SI * 2),
              list(min = Q_RP / 2, max = Q_RP * 2),
              list(min = Q_SP / 2, max = Q_SP * 2),
              list(min = G_SYN_L / 2, max = G_SYN_L * 2),
              list(min = G_SYN_SI / 2, max = G_SYN_SI * 2),
              list(min = k_L_GLOS / 2, max = k_L_GLOS * 2),
              list(min = k_SI_GLOS / 2, max = k_SI_GLOS * 2),
              list(min = init_GSH_L / 2, max = init_GSH_L * 2),
              list(min = init_GSH_SI / 2, max = init_GSH_SI * 2),
              list(min = k_GSH / 2, max = k_GSH * 2),
              list(min = k_DNA / 2, max = k_DNA * 2),
              list(min = C_PRO_L / 2, max = C_PRO_L * 2),
              list(min = C_PRO_SI / 2, max = C_PRO_SI * 2),
              list(min = C_L_dG / 2, max = C_L_dG * 2),
              list(min = T_0.5 / 2, max = T_0.5 * 2),
              list(min = DOSE / 1, max = DOSE * 1),
              list(min = Ka / 2, max = Ka * 2),
              list(min = Km_L_CA / 2, max = Km_L_CA * 2),
              list(min = Km_L_GST / 2, max = Km_L_GST * 2),
              list(min = Km_L_GST_G / 2, max = Km_L_GST_G * 2),
              list(min = Vsmax_L_CA / 2, max = Vsmax_L_CA * 2),
              list(min = Vsmax_L_AO / 2, max = Vsmax_L_AO * 2),
              list(min = Vsmax_L_GST / 2, max = Vsmax_L_GST * 2),
              list(min = Vsmax_L_GST_G / 2, max = Vsmax_L_GST_G * 2),
              list(min = Km_SI_CA/ 2, max = Km_SI_CA * 2),
              list(min = Km_SI_AO/ 2, max = Km_SI_AO * 2),
              list(min = Km_SI_OH/ 2, max = Km_SI_OH * 2),
              list(min = Km_SI_GST/ 2, max = Km_SI_GST * 2),
              list(min = Km_SI_GST_G/ 2, max = Km_SI_GST_G * 2),
              list(min = Vsmax_SI_CA/ 2, max = Vsmax_SI_CA * 2),
              list(min = Vsmax_SI_AO/ 2, max = Vsmax_SI_AO * 2),
              list(min = Vsmax_SI_GST/ 2, max = Vsmax_SI_GST * 2))
