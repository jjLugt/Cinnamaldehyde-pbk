...
times=seq(0,8,by=0.001) #from 0 to 8 hours in increments of 0.01 h
...
PBK_Cinnamaldehyde=function(t,state,parameters){
  with(as.list(c(state,parameters)),{
   ...
   C_OH_V_F       <- C_OH_F    / P_OH_F                 #Concentration of Cinnamyl alcOHol in venous blood leaving Fat in umol/l
    ...
    list(c(dA_GI,dA_V,dA_OH_V,dA_F,dA_OH_F,dAM_L_CA,dAM_L_AO,dAM_L_AG_GST,dAM_L_AG_CHEM,dAM_L_AP,dAM_L_DA_FORM,dAM_L_DA, 
...
#Running the model---------------------------------------------------------------------------------------------------------------------------------- 
PBK_Cinnamaldehyde_results = ode(y= state,times=times,func=PBK_Cinnamaldehyde, parms = parameters, hini = 0.001, method = "euler")
